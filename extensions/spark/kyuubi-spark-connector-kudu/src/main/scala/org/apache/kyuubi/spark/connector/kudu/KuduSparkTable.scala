...
class KuduSparkTable(kuduTable: KuduTable) extends SparkTable with SupportsRead with SupportsWrite {
  ...
  override val name: String = kuduTable.getName

  override def schema: StructType = KuduSchemaUtils.toSpark(kuduTable.getSchema)

  override def capabilities: util.Set[TableCapability] = Set(BATCH_READ, BATCH_WRITE).asJava
